"""Scrolling level + gain-reduction history display (premium-comp / premium-limiter style).
...
With interactive=True the reference line becomes draggable (premium-comp/premium-limiter
...
def level_history_js(
    *,
    ...
    return design_system_js() + "\n" + BYPASS_DIM_JS + "\n" + _JS_TEMPLATE.substitute(
        ...
        "//__PLOT_GEOMETRY__//",
        plot_geometry_short_js(
            "24", "mgraphics.size[0] - 22", "6", "mgraphics.size[1] - 6"
        ),
    ).replace("//__POINTER_Y__//", POINTER_Y_JS).replace(
        "//__POINTER_X__//", POINTER_X_JS
...
// areas, so they read as energy (a premium-limiter look) instead of a flat wash.
...
function paint() {
    ...
    // the GR-band hot edge and the reference-line flare — the premium-limiter/premium-comp
    ...
    // rides the cursor's level (premium-comp/premium-limiter threshold feedback); idle it's a
    // subtle tag so the value is always legible.
    if (ref_db !== null && ref_db >= lo_db && ref_db <= hi_db) {
        y = db_to_y(ref_db);
        var big = (INTERACTIVE && dragging) ? 1 : 0;

        // Flare: the reference line IGNITES with gain reduction (the premium-limiter/premium-comp
    ...
    if (peak_gr < -0.05) {
        ...
        // inset + premium-limiter/premium-comp convention (peak_gr is the deepest, negative).
...
function apply_ref(y, fine) {
    ...
    // Shift = fine-adjust (premium-EQ/premium-comp tactile grammar): move only 1/5th of the
