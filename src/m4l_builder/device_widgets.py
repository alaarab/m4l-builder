"""Composite UI-widget builders for Device — the heavy add_* methods
extracted from the Device god class. Mixed into Device via the MRO, so
``device.add_step_bars(...)`` etc. are unchanged (same call, same output)."""
from __future__ import annotations

from .parameters import ParameterSpec


class CompositeWidgetsMixin:
    ...
    def add_mode_glyph_selector(
        ...
        *,
        ...
        """Add a compact row of hand-drawn GLYPH buttons (a premium device's mode-glyphs)
    ...
    def add_custom_slider(
        ...
        *,
        ...
        track and the handle JUMPS to and tracks the cursor — pro-plugin behavior;
    ...
    def add_drag_curve_node(
        ...
        *,
        ...
        params (a premium-EQ band / compressor threshold-ratio dot / filter
    ...
    def add_envelope_editor(
        ...
        *,
        ...
        automatable ``live.dial`` params (catalog kit engine #1 — the
        Auto-Gate / FM-synth envelope-editor look).
    ...
    def add_curve_editor(
        ...
        *,
        ...
        (catalog kit engines #2-#6 — the breakpoint-curve core widget).
        ...
        **Persistence choice (investigated, honest):** a parameter-enabled
        ...
        hosts (Parametric EQ precedent), which is ALSO exactly how a boutique
        device ships editor state (its devices carry dozens of
        ...
        **Save path:** jsui ``points`` list -> ``route points`` ->
        ...
        # (the corpus ships `listfunnel 1` + `spray 12` with exactly this shape).
    ...
    def add_step_bars(
        ...
        *,
        ...
        hidden ``live.dial`` parameter hosts (catalog kit engines
        #7-#8 — a gate/random step lane + value-link chip).
    ...
    def add_bpatcher_module(
        ...
        *,
        ...
        channel-strip processor, as in commercial channel-strip devices). ``embed=1`` bakes the
    ...
    def add_component_rack(
        ...
        *,
        ...
        the corpus P11 rack (grounded in measured device conventions: six ``bpatcher args=[i]``
    ...
    def add_viz_bus(
        ...
        *,
        ...
        live displays; the standard viz-bus contract).
        ...
        ``---``-resolved buffer~ objects (the standard top-patcher pattern);
