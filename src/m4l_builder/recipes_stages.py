"""Recipes: stages — split out of recipes.py; re-exported by m4l_builder.recipes."""
...
def parametric_eq_band_backend(
    ...
    *,
    ...
    motion_depth_expr_id = device.add_newobj(
        ...
        "expr ($f2 > 0.5) * (($f1 / 100.) * 1.25 * cos($f3 * 0.0174533))",
    ...
    motion_gain_depth_expr_id = device.add_newobj(
        ...
        "expr ($f2 > 0.5) * (($f1 / 100.) * 12. * sin($f3 * 0.0174533))",
        numinlets=3, numoutlets=1,
        outlettype=[""],
...
def mc_poly_spine(device, id_prefix, *, voices=16, gen_name=None, gen_inlets=2,
    ...
    that premium synth devices use (``@voices 16`` / ``@mpemode 1 @voices 8 @steal 1``).
    Topology + port indices are VERBATIM from those devices.
    ...
    Args:
        ...
        mpe: emit ``@mpemode 1`` on the noteallocator (the MPE path).
        steal: emit ``@steal 1`` (voice stealing).
        autogain: ``@autogain 1`` on the mixdown (default 0, like the corpus default).
...
def beat_phase_gate(device, id_prefix="bpg", *, x=30, y=30):
    ...
    LiveAPI round-trip; corpus precedent).
...
def sample_drop_target(device, id_prefix, buffer_box_id, drop_rect, *,
    ...
    # An EMPTY live.drop (fresh device, or banged for restore before a file is
    # set) emits the sentinel symbol `none`. Left ungated it becomes `replace
    # none` -> `buffer~: none: can't open` in the Max console on every load. Drop
    # the sentinel here so only real paths reach `replace` (matched "none" exits
    # the left outlet unconnected; anything else passes through the right outlet).
    noneguard_id = device.add_newobj(
        f"{p}_noneguard", "sel none", numinlets=2, numoutlets=2,
        outlettype=["bang", ""], patching_rect=[x + 70, y + 10, 80, 20],
    )
    ...
    device.add_line(trig_id, 1, noneguard_id, 0)      # gate the `none` sentinel
    device.add_line(noneguard_id, 1, replace_id, 0)   # real paths only -> replace
