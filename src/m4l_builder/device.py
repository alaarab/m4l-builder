...
class Device(GraphContainer):
    ...
    def add_compiled_display(
        ...
        *,
        ...
        Z-ORDER depends on the compiled object: ``spectroscope~`` renders BELOW a
        v8ui, so the overlay goes in FRONT (default) and is transparent. ``scope~``
        renders ABOVE a v8ui, so set ``overlay_behind=True`` — the overlay is
        added FIRST (behind) and draws its own opaque ``overlay_bgcolor``, and the
        compiled object is made transparent (pass a transparent ``bgcolor`` in
        ``attrs``) so the overlay's graticule shows through. (Two boxes sharing the
        EXACT same rect z-fight — keep a small ``inset`` so the rects differ.)
