...
class Theme:
    ...
    # Second accent — the SELECTION/secondary color (a pink-on-cyan selection).
    ...
    def __post_init__(self):
        ...
        if self.tab_bg_on is None:
            ...
            # color, a pink-on-cyan selection), the primary accent stays for dials/
        ...
        if self.spectrum_color is None:
            # Spectra read best NEUTRAL grey (not the accent) — the EQ8 look.
...
# color; accent2 = selection/secondary (a pink-on-cyan selection idea).

COBALT = Theme(  # deep navy, cyan primary + pink selection
...
MAGMA = Theme(  # near-pure black, hot orange + crimson
...
NEBULA = Theme(  # violet + cyan (+ amber meters)
    bg=[0.055, 0.050, 0.072, 1.0],
    surface=[0.092, 0.084, 0.118, 1.0],
    section=[0.125, 0.114, 0.158, 1.0],
    text=[0.96, 0.98, 0.89, 1.0],        # near-white text
    text_dim=[0.52, 0.50, 0.58, 1.0],
    accent=[0.70, 0.42, 0.89, 1.0],      # violet primary
    accent2=[0.43, 0.83, 1.0, 1.0],      # cyan secondary
    meter_cold=[0.43, 0.83, 1.0, 1.0],   # cyan
    meter_warm=[0.96, 0.83, 0.16, 1.0],  # yellow
    meter_hot=[1.0, 0.71, 0.20, 1.0],    # amber (3rd accent)
...
# One accent per device (a single disciplined accent, never a
# rainbow). Values carried from the shipping build.py files so adopting
...
# the same meaning on every device, so a user reads a row's function
