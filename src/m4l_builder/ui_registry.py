...
DEVICE_WIDGET_SPECS = {
    ...
    "scope": DeviceWidgetSpec(
        ...
        # across the whole real-device corpus (SEAR etc.): only linecolor,
    ...
    "meter": DeviceWidgetSpec(
        ...
        theme_mapping={
            ...
            # (AS_Console) sets these to a mid-grey ~0.5 = the text_dim token, so the
    ...
    # strip = accent2.
    # live.step / live.grid (sequencer lanes, no synced doc — grounded on the J74
    # SliceShuffler + Roulette corpus). Map only the SOLID (alpha=1.0) attrs to tokens:
