...
grounded in standard granular + reverb DSP idioms. Place a returned function-definition string near the top of
...
__all__ = [
    "viz_declares", "viz_poke_block", "poly_mod_engine",
...
# ── D4 viz bus: gen -> named buffer~ -> jsui (a standard idiom for live
# displays). The gen
...
def viz_poke_block(body: str, *, refresh_ms: float = 40.0,
    ...
    """The counter-gated GUI tick: every
...
# rows so ONE function serves N voices (a polyrhythmic modulation cluster).
...
def poly_mod_engine(*, voices: int = 8, gui_refresh_ms: float = 40.0,
    ...
    channel at the lane clock (the function-editor feeds it
...
# The granular voice scatter (interval-gated round-robin scheduler +
...
def granular_voice_fn() -> str:
    """Return the gen function-definition for the granular voice scatter: an interval-gated round-robin grain scheduler + a
...
def freeze_capture_block(*, freeze_param: str = "freezeTgt",
    ...
    """The freeze gesture for a granular caller: on the RISING
    edge of ``freeze_param``, copy the last ``capture_samps`` of both Delay
    ``lines`` TIME-REVERSED into ``data`` (channels 0/1 — ``index =
...
def compose_gen_code(*, params=(), functions=(), body: str = "") -> str:
    ...
    TOP of the codebox; a ``Param``/declaration BEFORE a function def silences the whole codebox
...
# An asymmetric History smoother:
...
def rampsmooth_fn() -> str:
    ...
    Place at the top of GEN_CODE, then call
    e.g. ``smoothed = rampsmooth(target, 480, 480);``.
    """
    return RAMPSMOOTH_FN


# A 2-pole RBJ lowpass biquad (Robert Bristow-Johnson's Audio EQ Cookbook)
...
# built-in CPU-discipline pattern). DF1 with 4 history taps. The y-feedback
# write (his_h4) is denormal-flushed
...
# A Schroeder/Dattorro allpass diffuser. The
...
# The compact one-multiply Schroeder allpass
...
def diffuse_fn() -> str:
    ...
    Chain several (mutually-prime delays) for reverb diffusion."""
    return DIFFUSE_FN


# The RBJ 2-pole HIGHPASS twin of lowpass_12
# (same DF1 + smoothed coeffs + change()-gated recompute). Use for the channel
# strip HP and any EQ low-cut. Same denormal flush as lowpass_12: the his_h4
# y-feedback write is fixdenorm'd.
...
def allpass_fn() -> str:
    ...
    Chain N of these for a reverb tail."""
    return ALLPASS_FN
...
def ring_delay(name: str, size_samps, *, input: str = "x", delay: str = "dsamps",
    ...
    standard poke/peek/wrap ring-buffer idiom. ``size_samps`` is the buffer length.
...
# A drive-able logistic-sigmoid saturation curve whose knee
...
# The curve is the single line `y = (2/(1+exp(sigmoid*x))) - 1`.
...
def variable_sigmoid_fn() -> str:
    ...
    saturator (pre/post gain in dB, knee set by ``sigmoid``): ``sigmoid``=-2 approximates tanh, -5 is tape-like."""
    return VARIABLE_SIGMOID_FN


# Constants for the modulated allpass reverb. The pre-diffusion bank is 4 series Schroeder allpasses with
...
def modulated_allpass_reverb(
    *,
    ...
    The topology is a 4-stage series
    ...
    # ~3.5 s of room, so declare at 5 s.
    decls = []
    for ch in ("L", "R"):
    ...
    # executable statement (gen requires every operator declared up front); a
...
# ===========================================================================
# D3 — synth-voice kit (FM operator, wavetable LFO, envelopes, SVF)
# ===========================================================================

# The FM operator with phase-modulation input + self-feedback — a
# full synth voice in one function (Chowning FM). `pm` is the phase-mod input (chain operators by
...
# Zero attack-or-decay holds at 1 (a gate).
...
# Zavalishin's TPT design; Émilie Gillet's MIT implementation.
...
def op_fn() -> str:
    ...
    — a classic FM operator (Chowning FM) with a phase-modulation input and
    signed self-feedback. Chain operators by feeding output into the next
    ``pm``; ``trig`` resets phase at a new note."""
    return OP_FN


def qpow_fn() -> str:
    """Return the gen function-definition for ``qpow(x, p)`` — an iterative
    power-curve waveshaper (signed ``p``), used for parameter curves and
...
def fade_env_fn() -> str:
    ...
    AD gate envelope (rate-per-second one-pole slews). This is
...
def ramp_env_fn() -> str:
    ...
    a per-note-jitterable 1→0 ramp/scheduler clock."""
    return RAMP_FN


def tanh_approx_fn() -> str:
    """Return the gen function-definition for ``tanA(x)`` — a fast Padé
    tanh approximation. Required by ``svf_fn``."""
    return TANA_FN
...
def svf_fn() -> str:
    ...
    LP↔HP. MUST be emitted together with ``tanh_approx_fn``
    (it calls ``tanA``)."""
    return SVF_FN
...
def lfo_osc_fn() -> str:
    ...
    — a wavetable LFO with ``qpow`` phase-distortion morph.
