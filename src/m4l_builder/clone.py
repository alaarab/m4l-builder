...
each high-level primitive; this engine is the structural backbone underneath it.
    ...
    device = clone_from_patcher_json("research/amxd_corpus/_json/AS_Console_Norm.json",
                                     name="AS Console Norm Rebuild")
...
def clone_from_patcher_json(json_path, name=None, *, device_type="audio_effect") -> Device:
    ...
    with tolerant_reconstruction():
        for box in boxes:
            device.add_box(box)
    for line in lines:
        src_pl = line["patchline"]["source"]
        dst_pl = line["patchline"]["destination"]
