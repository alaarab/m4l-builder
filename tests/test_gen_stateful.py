...
def test_rampsmooth_is_a_valid_gen_function():
    ...
    # balanced braces + the History-smoother shape (verbatim from stranular)
    assert code.count("{") == code.count("}")
...
def test_variable_sigmoid_is_verbatim_chiral_tapeh():
...
def test_modulated_allpass_reverb_grounded_topology():
    ...
    assert code.count("{") == code.count("}")
    # Particle-Reverb constants, verbatim: pre-diffusion delay seeds, the 1.36255
...
# --- D3: Superberry synth-voice kit (verbatim) -----------------------------
