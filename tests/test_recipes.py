...
class TestSampleDropTarget:
    ...
    def test_intake_chain_present(self):
        ...
        assert "sel none" in texts     # empty-Blob `none` sentinel guard

    def test_wires_replace_into_buffer_and_settle(self):
        device, _ = self._device()
        pairs = _line_pairs(device)
        assert ("smp_drop", "smp_trig") in pairs
        # path is gated by `sel none` before reaching replace (drops the empty-
        # Blob `none` sentinel that would otherwise fire `replace none`).
        assert ("smp_trig", "smp_noneguard") in pairs
        assert ("smp_noneguard", "smp_replace") in pairs
...
class TestMcPolySpine:
    ...
    def test_mpe_and_voice_count_options(self):
        ...
        texts = _box_texts(device)
        # A premium synth's MPE allocator grammar + a named .gendsp voice
...
class TestTwoPanelStrip:
    def test_grounded_as_console_geometry(self):
        ...
        # a premium two-panel channel-strip grammar: wide main + narrow side, gap 2
...
class TestBypassWrapper:
    def test_selector_switch_with_shared_toggle(self):
        device = AudioEffect("test", width=200, height=120)
        device.add_live_text("my_toggle", "Bypass", [0, 0, 50, 16], mode=1)
        res = bypass_wrapper(device, "comp", toggle="my_toggle",
                             wet_source=lambda ch, ci: ("wet_gen", ci))
        texts = _box_texts(device)
        assert "selector~ 2 1" in texts
        assert "+ 1" in texts                 # toggle 0/1 -> control 1/2 (1-indexed)
        pairs = _line_pairs(device)
        assert ("my_toggle", "comp_add1") in pairs
        assert ("comp_add1", "comp_sel_l") in pairs
        assert ("comp_add1", "comp_sel_r") in pairs
        assert ("wet_gen", "comp_sel_l") in pairs
        assert ("obj-plugin", "comp_sel_l") in pairs     # default dry source
        assert ("comp_sel_l", "obj-plugout") in pairs    # default audio_out
        assert res["selector"] == {"l": "comp_sel_l", "r": "comp_sel_r"}

    def test_custom_dry_source_and_audio_out(self):
        device = AudioEffect("test", width=200, height=120)
        res = bypass_wrapper(device, "eq", toggle="tog",
                             wet_source=lambda ch, ci: (f"mix_{ch}", 0),
                             dry_source=lambda ch, ci: ("pre_gen", 4 + ci),
                             audio_out="custom_out")
        pairs = _line_pairs(device)
        assert ("mix_l", "eq_sel_l") in pairs
        assert ("pre_gen", "eq_sel_l") in pairs           # custom dry tap
        assert ("eq_sel_l", "custom_out") in pairs
        assert res["shim"] == "eq_add1"
...
class TestModeStack:
    """Generator mode-stack container (catalog item #9): a vertical enum tab
...
class TestPageSelector:
    """Vertical page tab; rows hidden via manage= script sendbox (catalog #25)."""

    def test_tab_and_manage(self):
        from m4l_builder import AudioEffect
        from m4l_builder.recipes import page_selector
        device = AudioEffect("pg", width=300, height=168)
        page_selector(device, "pg", at=[4, 40], n_pages=3, rows_per_page=3,
                      manage={"row_1": 1, "row_2": 2})
        boxes = {b["box"]["id"]: b["box"] for b in device.boxes}
        tab = boxes["pg_tab"]
        assert tab["maxclass"] == "live.tab"
        assert tab["saved_attribute_attributes"]["valueof"]["parameter_enum"] \
            == ["1", "2", "3"]
        lines = {(ln["patchline"]["source"][0],
                  ln["patchline"]["destination"][0]) for ln in device.lines}
        # load re-broadcast via live.thisdevice (never loadbang)
        assert ("pg_td", "pg_tb") in lines and ("pg_tb", "pg_tab") in lines
        # manage= hiding path: tab -> per-row window expr -> `script sendbox
        # <varname> hidden $1` -> parent-side thispatcher
        assert boxes["pg_mm0"]["text"] == "script sendbox row_1 hidden $1"
        assert ("pg_tab", "pg_mx0") in lines
        assert ("pg_mx0", "pg_mm0") in lines and ("pg_mm0", "pg_this") in lines
        # the removed pagewin broadcast chain leaves no pg_pre/pg_pk/pg_lo/pg_t
        for gone in ("pg_pre", "pg_pk", "pg_lo", "pg_lo2", "pg_hi", "pg_t"):
            assert gone not in boxes

    def test_slot_has_no_page_machinery(self):
        # the broken slot-side pages= broadcast (unsubstituted `#1` -> Max
        # 'expr: bad string #1' at load, thispatcher-in-bpatcher that never
        # hid) was removed; a built slot carries no slot_page* boxes and
        # pages= is no longer an accepted kwarg.
        import pytest

        from m4l_builder import AudioEffect
        from m4l_builder.recipes import modulator_slot_component
        device = AudioEffect("pg", width=300, height=168)
        sub, _ = modulator_slot_component(device, accent=[1, 0.6, 0.2, 1])
        assert not any(b["box"]["id"].startswith("slot_page")
                       for b in sub.boxes)
        with pytest.raises(TypeError):
            modulator_slot_component(AudioEffect("x", 300, 168),
                                     accent=[1, 0.6, 0.2, 1], pages=True)
...
class TestStandardChip:
    ...
    def test_glyphs_registered(self):
